...
2. 页面不再直接编辑底层 API 参数，只在需要时轻量切换“当前页使用哪个模型预设”。
...
7. 系统保存一个全局默认预设；页面如单独选择过预设，则持久覆盖该页默认行为。
...
页面不再放一整块显眼的 `当前模型预设` 配置区。默认直接继承全局默认模型，避免在多个页面重复出现同一套选择器。

### 11.2 轻量入口

需要模型的页面在标题区或局部工具栏放一个很轻的小入口，展示当前生效模型名。点击后再展开：
...
页面一旦手动改过模型，就将该选择记录为页面级覆盖，并在下次进入该页面时继续使用。
...
1. 页面级覆盖 `pageKey -> presetId`
2. 全局默认 `presetId`
3. 系统内第一个可用默认预设

### 13.2 首次初始化

首次生成默认预设时，只写入一个全局默认 `presetId`，不再为每个页面预先写死默认选择。
...
2. 若存在则使用该模型
3. 若不存在则跟随全局默认
4. 若全局默认缺失，则回退系统可用默认预设
5. 若系统也没有任何可用预设，则提示用户去“模型预设管理”补配置
...
4. 用户可复制预设。
5. 全局默认模型可保存并恢复。
6. 页面可独立覆盖全局默认，且刷新后可恢复。
7. 不满足页面能力要求的预设会被禁用并提示原因。
8. 旧 `llm1/llm2` key 会被清理，不再参与新系统。
9. 现有调用链不回归。
10. `npm test` 通过。
11. `npm run build` 通过。
